...
const articleUrl = (entry) => `${basePath}/read/?id=${encodeURIComponent(entry.id)}`;
...
export default function ArticleReader({ entry }) {
    ...
    <div className="article-page" id="top">
      ...
      <main className="article-layout">
        <aside className="article-sidebar">
          <div className="article-sidebar-card"><span>CONTENTS</span><nav>{headings.map((heading, index) => <a className={`toc-depth-${heading.depth}`} key={`${heading.id}-${index}`} href={`#${heading.id}`}><em>{String(index + 1).padStart(2, "0")}</em>{heading.title}</a>)}</nav></div>
          <div className="article-sidebar-card export-card"><span>DOWNLOAD</span><h2>保存这篇文章</h2><p>下载后可以继续批注、编辑或离线阅读。</p><ArticleDownloads entry={entry} basePath={basePath} /></div>
        </aside>
        <article className="article-reader">
          ...
          {!!entry.attachments?.length && <section className="article-file-section" id="attachments"><div className="article-section-heading"><span>附</span><h2>相关资料与附件</h2></div><div className="article-file-list">{entry.attachments.map((attachment) => <a key={attachment.path} href={assetUrl(attachment.path)} target="_blank" rel="noreferrer" download={attachment.download !== false}><strong>{attachment.name}</strong><small>{attachment.description || "打开或下载资料"}</small><span>下载 ↓</span></a>)}</div></section>}
          <div className="article-citation"><span>推荐引用</span><p>{entry.citation}</p></div>
          <footer className="article-reader-footer"><a href={`${basePath}/#library`}>返回全部文章</a>{nextEntry && <a href={articleUrl(nextEntry)}>下一篇：{nextEntry.title} →</a>}</footer>
        </article>
      </main>
